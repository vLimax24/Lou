...
export const getUpcomingProjects = authQuery({
  args: {},
  handler: async ({ db, user }) => {
    const projects = await db
      .query("projects")
      .filter(q => q.gt(q.field("deadline"), new Date().toISOString()))
      .collect()
...
export const searchRessources = authQuery({
  ...
  handler: async (ctx, args) => {
    ...
    const filteredRessources = arr.filter((ressource: string) => {
      return ressource.toLowerCase().includes(args.searchTerm.toLowerCase())
    })
