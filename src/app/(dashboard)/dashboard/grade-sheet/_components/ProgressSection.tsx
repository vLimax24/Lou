...
const ProgressSection = () => {
    ...
    useEffect(() => {
        ...
        grades.forEach(grade => {
            ...
            if (selectedBadges.length === 0 || grade.badges.some(badge => selectedBadges.includes(badge))) {
        ...
        const result: ChartData[] = Object.entries(aggregatedData).map(([month, subjects]: any) => {
            ...
            for (const [subject, gradesArray] of Object.entries<any>(subjects)) {
                const totalAverage = gradesArray.reduce((acc: any, curr: any) => acc + curr, 0) / gradesArray.length || 0
            ...
            <div className="w-4/5 md:w-3/5">
                {chartData.length > 0 && (
                    <LineChart
                        ...
                        colors={["indigo", "rose", "amber", "slate", "gray", "fuchsia", "zinc", "neutral", "stone", "red", "orange", "yellow", "lime", "emerald", "teal", "cyan", "sky", "blue", "indigo", "violet", "purple", "pink", "rose"]}
                        yAxisWidth={32}
    ...
    <div className="mt-4">
        {chartData.length > 0 && (
            <>
                <p>🎓 Grade: {chartData.find((data: any) => data.month === removeQuotes(JSON.stringify(value?.month)))?.[value.categoryClicked] || 0}</p>
                {chartData.length > 1 && (
                    <>
                        {chartData.findIndex((data: any) => data.month === removeQuotes(JSON.stringify(value?.month))) > 0 && (
                            <>
                                <p>⬅️ Previous Grade: {chartData[chartData.findIndex((data: any) => data.month === removeQuotes(JSON.stringify(value?.month))) - 1][value.categoryClicked] || 0}</p>
                                <p>
                                    {chartData.find((data: any) => data.month === removeQuotes(JSON.stringify(value?.month))) &&
                                        chartData[chartData.findIndex((data: any) => data.month === removeQuotes(JSON.stringify(value?.month))) - 1] &&
                                        parseFloat(chartData.find((data: any) => data.month === removeQuotes(JSON.stringify(value?.month)))?.[value.categoryClicked] || 0) >
                                        parseFloat(chartData[chartData.findIndex((data: any) => data.month === removeQuotes(JSON.stringify(value?.month))) - 1][value.categoryClicked] || 0) ?
                                ...
                                <p>
                                    {chartData.find((data: any) => data.month === removeQuotes(JSON.stringify(value?.month))) &&
                                        chartData[chartData.findIndex((data: any) => data.month === removeQuotes(JSON.stringify(value?.month))) - 1] &&
                                        parseFloat(chartData.find((data: any) => data.month === removeQuotes(JSON.stringify(value?.month)))?.[value.categoryClicked] || 0) ===
                                        Math.max(...chartData.map((data: any) => parseFloat(data[value.categoryClicked] || 0))) ?
                                        `🔥 Congratulations! This is the best month for ${removeQuotes(value.categoryClicked)} so far.` :
                                        parseFloat(chartData.find((data: any) => data.month === removeQuotes(JSON.stringify(value?.month)))?.[value.categoryClicked] || 0) ===
                                        Math.min(...chartData.map((data: any) => parseFloat(data[value.categoryClicked] || 0))) ?
                                ...
                                <p>
                                    ➡️ You have received grades for {chartData.length} months for {removeQuotes(value.categoryClicked)} so far.
                                </p>
                                <p>
                                    ⚖️ The average grade for {removeQuotes(value.categoryClicked)} over the available months is {(
                                        chartData.reduce((acc: any, data: any) => acc + parseFloat(data[value.categoryClicked] || 0), 0) / chartData.length
                                    ).toFixed(2)} GPA.
                                </p>
                                <p>
                                    📊 The standard deviation of grades for {removeQuotes(value.categoryClicked)} is {Math.sqrt(
                                        chartData.reduce((acc: any, data: any) => acc + Math.pow(parseFloat(data[value.categoryClicked] || 0) - (
                                            chartData.reduce((acc: any, data: any) => acc + parseFloat(data[value.categoryClicked] || 0), 0) / chartData.length
                                        ), 2), 0) / chartData.length
                                    ).toFixed(2)}.
                                </p>
                            </>
                        )}
            ...
            <Card className="w-2/5 ml-5 p-5 bg-transparent border-none flex flex-col shadow-none">
                <CardTitle className="mb-2">Filter graph by badges</CardTitle>
                <div className="flex flex-col">
                    <div className="flex mt-3">
                            {uniqueBadges.map(badge => (
                                <div key={badge} className="rounded-3xl border py-1.5 pl-2 pr-4 border-primaryGray mr-3 hover:cursor-pointer flex items-center" onClick={() => handleBadgeClick(badge)}>
                                    <Checkbox id={badge} className="mr-2 size-5 rounded-full" checked={selectedBadges.includes(badge)}/>
                                    {badge}
                                </div>
                            ))}
                    </div>
